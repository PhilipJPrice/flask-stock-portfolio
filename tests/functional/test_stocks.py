...
def test_get_stock_list_logged_in(test_client, add_stocks_for_default_user, mock_requests_get_success_daily):
    ...
    """
    headers = [b'Stock Symbol', b'Number of Shares', b'Share Price', b'Purchase Date', b'Current Share Price', b'Stock Position Value', b'TOTAL VALUE']
...
def test_monkeypatch_get_success(monkeypatch):
    ...
    assert '2023-02-02' in r.json()['Meta Data']['3. Last Refreshed']
    assert '264.6000' in r.json()['Time Series (Daily)']['2023-02-02']['4. close']
...
def test_stock_detail_page(test_client, add_stocks_for_default_user, mock_requests_get_success_weekly):
    """
    GIVEN a Flask application configured for testing, with the default user logged in
        default set of stocks in the database
...
def test_stock_detail_page_failed_response(test_client, add_stocks_for_default_user, mock_requests_get_failure):
    """
    GIVEN a Flask application configured for testing, with the default user logged in
        and the default set of stocks in the database
